...
export default function OrderForm() {
  ...
  const canSubmitOrder =
    name.trim().length > 1 && phone.trim().length > 6 && address.trim().length > 5;

  function handleOrderSubmit(e: FormEvent) {
    e.preventDefault();
    if (!result || !canSubmitOrder) return;
    ...
    const lines = [
      ...
      `Adres: ${address.trim()}`,
      "—",
      "Ücretsiz kapıdan alım-teslim için servis çağırıyorum.",
    ];
    const url = `${siteConfig.whatsappHref}?text=${encodeURIComponent(lines.join("\n"))}`;
  ...
  if (orderStep) {
    return (
      <form
        ...
        className="rounded-2xl border border-brand-100 bg-white p-6 shadow-sm sm:p-7"
      >
        <button
          type="button"
          onClick={() => setOrderStep(false)}
        ...
        <div className="mt-5 space-y-4">
          ...
          <label className="block text-sm">
            ...
            <textarea
              ...
              placeholder="Mahalle, cadde/sokak, bina/daire no, ilçe"
              value={address}
        ...
        <p className="mt-4 text-xs leading-relaxed text-brand-900/50">
          Fiyat tahminidir; ekibimiz halıyı gördükten sonra kesinleşir.
          Butona bastığınızda WhatsApp üzerinden sipariş talebiniz bize
          iletilir, size dönüş yaparak randevunuzu netleştiririz.
        </p>

        <button
          type="submit"
          disabled={!canSubmitOrder}
          className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-full bg-[#25D366] px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-[#25D366]/20 transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40"
        >
  ...
  return (
    <div className="rounded-2xl border border-brand-100 bg-white p-6 shadow-sm sm:p-7">
      ...
      <button
        ...
        className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-full bg-gold-400 px-8 py-3.5 text-sm font-bold text-brand-950 shadow-lg shadow-gold-400/25 transition hover:bg-gold-300 disabled:cursor-not-allowed disabled:opacity-40"
      >
        Sipariş Ver
      </button>
    </div>
